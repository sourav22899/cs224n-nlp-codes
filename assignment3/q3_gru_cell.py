...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable('U_r',shape = (self.state_size,self.state_size),initializer = tf.contrib.layers.xavier_initializer())
            W_r = tf.get_variable('W_r',shape = (self.input_size,self.state_size),initializer = tf.contrib.layers.xavier_initializer())
            b_r = tf.get_variable('b_r',shape = (self.state_size),initializer = init)
            U_z = tf.get_variable('U_z',shape = (self.state_size,self.state_size),initializer = tf.contrib.layers.xavier_initializer())
            W_z = tf.get_variable('W_z',shape = (self.input_size,self.state_size),initializer = tf.contrib.layers.xavier_initializer())
            b_z = tf.get_variable('b_z',shape = (self.state_size),initializer = init)
            U_o = tf.get_variable('U_o',shape = (self.state_size,self.state_size),initializer = tf.contrib.layers.xavier_initializer())
            W_o = tf.get_variable('W_o',shape = (self.input_size,self.state_size),initializer = tf.contrib.layers.xavier_initializer())
            b_o = tf.get_variable('b_o',shape = (self.state_size),initializer = init)
            z = tf.sigmoid(tf.matmul(inputs,W_z) + tf.matmul(state,U_z) + b_z)
            r = tf.sigmoid(tf.matmul(inputs,W_r) + tf.matmul(state,U_r) + b_r)
            o = tf.tanh(tf.matmul(inputs,W_o) + tf.multiply(r,tf.matmul(state,U_o)) + b_o)
            one = tf.ones_like(o)
            new_state = tf.multiply(z,state) + tf.multiply(one - z,o) 
            ### END YOUR CODE ###
